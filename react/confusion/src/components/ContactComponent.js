...
class Contact extends Component {
    ...
    handleInputChange(event){
        const target = event.target ; //事件发生对象
    ...
    handleSubmit(event){
        console.log("Current Stae is : " + JSON.stringify(this.state))
        alert("Current Stae is : " + JSON.stringify(this.state))
        event.preventDefault(); // go to the next page
    }

    //which fiel has been modified
                ...
                <div className= "row row-content">
                    ...
                    <div className="col-12 col-md-9">
                        <Form onSubmit = {this.handleSubmit}>
                            <FormGroup row>
                                <Label htmlFor ="firstname" md={2}>First Name</Label> {/**会混淆 jsx for 逻辑的for */}
                                <Col md= {10}>
                                    <Input type="text" id="firstname" name ="firstname" placeholder = "First Name" value = {this.state.firstname} onChange={this.handleInputChange} onBlur={this.handleBlur("firstname")}
                                           valid = {errors.firstname===""} invalid = {errors.firstname!==""}/>  {/**tie state*/}
                                    <FormFeedback> {errors.firstname} </FormFeedback>   {/**display*/}
                                   
                                </Col>
                            </FormGroup>

                            <FormGroup row>
                                <Label htmlFor ="lastname" md={2}>Last Name</Label> {/**会混淆 jsx for 逻辑的for */}
                                <Col md= {10}>
                            ...
                            <FormGroup row>
                                ...
                                <Col md= {10}>
                                    <Input type="email" id="telnum" name ="email" placeholder = "Email" value = {this.state.email} onChange={this.handleInputChange} onBlur={this.handleBlur("email")}
